/* ************************************************************************** */
/*                                                                            */
/*                                                        ::::::::            */
/*   player.c                                           :+:    :+:            */
/*                                                     +:+                    */
/*   By: rbraaksm <rbraaksm@student.codam.nl>         +#+                     */
/*                                                   +#+                      */
/*   Created: 2020/02/07 15:47:58 by rbraaksm      #+#    #+#                 */
/*   Updated: 2020/04/22 17:01:17 by rbraaksm      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */

#include "cub3d.h"

// void	rotate_player(t_vars *v, float rot)
// {
// 	v->ray.playdir += rot;
// 	if (v->ray.playdir < 0)
// 		v->ray.playdir += (2 * M_PI);
// 	if (v->ray.playdir > (2 * M_PI))
// 		v->ray.playdir -= (2 * M_PI);
// }

// void	move_player(t_vars *v, float move)
// {
// 	float temp_x;
// 	float temp_y;

// 	temp_x = v->player.x;
// 	temp_y = v->player.y;
// 	v->player.x += sin(v->ray.playdir) * move;
// 	if (v->map[(int)v->player.y][(int)v->player.x] == '1')
// 		v->player.x = temp_x;
// 	v->player.y += cos(v->ray.playdir) * move;
// 	if (v->map[(int)v->player.y][(int)v->player.x] == '1')
// 		v->player.y = temp_y;
// }

// void	crab_player(t_vars *v, float move, char c)
// {
// 	float temp_x;
// 	float temp_y;

// 	temp_x = v->player.x;
// 	temp_y = v->player.y;
// 	if (c == 'C')
// 	{
// 		v->player.x += sin(v->ray.playdir + M_PI / 2) * move;
// 		if (v->map[(int)v->player.y][(int)v->player.x] == '1')
// 			v->player.x = temp_x;
// 		v->player.y += cos(v->ray.playdir + M_PI / 2) * move;
// 		if (v->map[(int)v->player.y][(int)v->player.x] == '1')
// 			v->player.y = temp_y;
// 	}
// 	else
// 	{
// 		v->player.x += sin(v->ray.playdir) * move;
// 		if (v->map[(int)v->player.y][(int)v->player.x] == '1')
// 			v->player.x = temp_x;
// 		v->player.y += cos(v->ray.playdir) * move;
// 		if (v->map[(int)v->player.y][(int)v->player.x] == '1')
// 			v->player.y = temp_y;
// 	}
// }
